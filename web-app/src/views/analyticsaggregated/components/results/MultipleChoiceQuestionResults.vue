<template>
  <div class="flex flex-col gap-6">
    ...
    <div v-if="hasChartData">
      ...
      <!-- Chart Tabs -->
      <Tabs v-model="activeTab" value="absolute">
        <TabList>
          <Tab value="absolute">
            {{ $t('analytics_aggregated.choice_chart.bar_chart') }}
          </Tab>
          <Tab value="percentage">
            {{ $t('analytics_aggregated.choice_table.percentage') }}
          </Tab>
        </TabList>
        <TabPanels>
          <TabPanel value="absolute">
            <div ref="absoluteChartContainer" class="w-full h-96"></div>
          </TabPanel>
          <TabPanel value="percentage">
            <div ref="percentageChartContainer" class="w-full h-96"></div>
          </TabPanel>
        </TabPanels>
      </Tabs>
    </div>
...
const activeTab = ref<'absolute' | 'percentage'>('absolute');
const absoluteChartContainer = ref<HTMLElement>();
const percentageChartContainer = ref<HTMLElement>();
...
const hasChartData = computed(() => {
  ...
  return !!(
    chartData?.bar_chart ||
    (chartData as any)?.bar_percentage_chart ||
...
const createAbsoluteBarChart = async () => {
  if (!absoluteChartContainer.value) return;
  ...
  await Plotly.newPlot(absoluteChartContainer.value, plotData, layout, config);
};

const createPercentageBarChart = async () => {
  if (!percentageChartContainer.value) return;

  const chartData = props.questionData.analytics.chart_data;
  const percentageChart = (chartData as any)?.bar_percentage_chart;
  if (!percentageChart) return;

  // Handle both x and labels properties
  const xData = percentageChart.x || percentageChart.labels;
  const labels = xData.map((label: any) => formatMLString(label, locale.value));
  const values = percentageChart.y;
  const colors = getChartColors().slice(0, labels.length);
  ...
  const layout: any = {
    ...
    yaxis: {
      title: t('analytics_aggregated.choice_table.percentage'),
      range: [0, 100],
    },
    margin: { t: 20, b: 100, l: 60, r: 20 },
    font: {
...
onMounted(() => {
  ...
  window.addEventListener('resize', () => {
    resizeChart(absoluteChartContainer.value);
    resizeChart(percentageChartContainer.value);
  });
});
...
onUnmounted(() => {
  ...
  if (absoluteChartContainer.value) {
    Plotly.purge(absoluteChartContainer.value);
  }
  if (percentageChartContainer.value) {
    Plotly.purge(percentageChartContainer.value);
  }
});
</script>
