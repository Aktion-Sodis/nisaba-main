...
async function main(octokit) {
  ...
  console.log(pullRequestsOfLastCommit.map(({ labels }) => labels));

  if (shouldDeployBackend) return [...resArray, "admin-app", "analytics-app"];

  const shouldDeployAdminApp = pullRequestsOfLastCommit.some(({ labels }) =>
    labels.some(({ name }) => name === "deploy:admin-app")
  );

  const shouldDeployAnalyticsApp = pullRequestsOfLastCommit.some(({ labels }) =>
    labels.some(({ name }) => name === "deploy:analytics-app")
  );
