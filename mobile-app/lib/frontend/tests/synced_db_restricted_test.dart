...
class SyncedDBRestrictedTest extends StatelessWidget {
  ...
  Future<void> _initDBWithGraphQLDB() async {
    print("DBTest: Initializing SyncedDB");
    db = SyncedDB(LocalDB(), GraphQLDB());
  ...
  Future<void> _initDBWithLocalDB() async {
    print("DBTest: Initializing SyncedDB");
    db = SyncedDB(LocalDB(), LocalDB());
    (db as SyncedDB).syncUpstreamAutomatically = false;
    ...
    db.registerModel(
        ...
        SyncedDBModelRegistration(
            ...
            remoteDBModelRegistration: LocalDBModelRegistration(
              toDBModel: TestObject.fromJson,
            )));

    (db.localDB as LocalDB).initLocalDB("LocalDB_Test");
    (db.remoteDB as LocalDB).initLocalDB("LocalDB_Test2");

    print("DBTest: DB initialized");
    return Future.value();
  }

  // Restricted test works without update and delete mutations
  Future<void> _startUpstreamSyncTest() async {
    print("DBTest: Starting Upstream Sync Test");

    print("IMPORTANT: ENSURE THAT THE DB IS EMPTY");
    ...
    List<TestObject> objectsReceivedAfterSync =
        await db.remoteDB.get<TestObject>(TestObject);
    if (!listElementsEqual(_getAges(objectsReceivedAfterSync), [1, 2, 3])) {
      throw "DBTest: Wrong objects received from remoteDB after the second sync";
    }

    TestObject? receivedTestObject2 =
        await db.remoteDB.getById<TestObject>(TestObject, testObject2.id!);

    if (receivedTestObject2 == null) {
      throw "DBTest: The TestObject with id=testObject2.id has not been found";
    }
    if (receivedTestObject2.id != testObject2.id) {
      throw "DBTest: The received TestObject does not match the requested one";
    }

    var receivedQueueObject = await db.synchronizer.queue.get();
    if (receivedQueueObject != null) {
      throw "DBTest: The queue is not empty";
    }
  ...
  Future<void> _startDownstreamSyncTest() async {
    ...
    // UPDATE OPERATIOn IN NOT SUPPORTED YET
    /*testObject1.age = 4;
  ...
  Future<void> _clearDB() async {
    ...
    if (!(db.remoteDB is GraphQLDB)) {
      ...
      print(objectList2.length.toString() +
          " objects deleted from the remote DB");
    }
    ...
    return Scaffold(
        ...
        body: Scrollbar(
          child: ListView(
            ...
            children: [
              ElevatedButton(
                onPressed: _initDBWithLocalDB,
                child: const Text("Init DB with only LocalDB"),
              ),
              SizedBox(height: 20),
              ElevatedButton(
                onPressed: _initDBWithGraphQLDB,
                child: const Text("Init DB with GraphQLDB"),
              ),
              SizedBox(height: 20),
              ElevatedButton(
