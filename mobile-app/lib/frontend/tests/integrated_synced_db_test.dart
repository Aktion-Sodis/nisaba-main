...
class IntegratedSyncedDBTest extends StatelessWidget {
  ...
  Future<void> _printAllEntriesInRemoteDB() async {
    print("Printing all entries in the remote DB");
    List<TestObject> objectList = await db.remoteDB.get<TestObject>(
      TestObject,
    );
        ...
        body: Scrollbar(
          child: ListView(
            ...
            children: [
              ...
              SizedBox(height: 20),
              ElevatedButton(
                onPressed: _printAllEntriesInRemoteDB,
                child: const Text("Print all entries in the remote DB"),
              ),
            ],
          ),
        ));
