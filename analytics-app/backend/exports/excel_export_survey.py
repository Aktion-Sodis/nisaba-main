...
def string_from_Question_Option_Answers(question_options):
    ...
    for question_option in question_options:
        ...
        non_empty_language_text = [language_text for language_text in question_option['text']['languageTexts'] if language_text != ""]
...
def create_pandas_frames_for_export(survey, executed_surveys, entities):
    ...
    executed_survey_executor = [esurvey['whoExecutedIt']['firstName'] + " " + esurvey['whoExecutedIt']['lastName'] for esurvey in executed_surveys]

    executed_survey_frame = pd.DataFrame({'executor': executed_survey_executor}, index=executed_survey_ids)
    ...
    executed_survey_longitude = [esurvey['location']['longitude'] for esurvey in executed_surveys]
                ...
                for answer in esurvey["answers"]:
                    if answer["questionID"] == question["id"]:
                        if answer.get('questionOptions', None)!= None:
                            question_answers.append(string_from_Question_Option_Answers(answer['questionOptions']))
                        else:
...
def get_entity_ids_from_surveys(executed_surveys):
    ...
    for executed_survey in executed_surveys:
        entities.append(executed_survey["appliedIntervention"]["entityAppliedInterventionsId"])
    return entities
